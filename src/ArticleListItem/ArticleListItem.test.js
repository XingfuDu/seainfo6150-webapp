...
describe('ArticleListItem tests', () => {
	...
	it('renders correctly after button is clicked for show less', () => {
		const { container, getByText } = render(<ArticleListItem article={article} key={article.slug} />);
	...
	it('renders correctly after button is clicked for show more', () => {
		const { container, getByText } = render(<ArticleListItem article={article} key={article.slug} />);
